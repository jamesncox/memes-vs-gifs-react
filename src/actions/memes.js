...
export function sendMemeRequest(sendObj) {
    ...
    return (dispatch) => {
        fetch("https://memes-vs-gifs-api.herokuapp.com/caption_joins", {
            method: "POST",
            headers: {
...
export const getSavedMemes = () => {
    return async dispatch => {
        ...
        try {
            const res = await fetch("https://memes-vs-gifs-api.herokuapp.com/memes")
            if (!res.ok) {
                throw res
            }
...
export function deleteMeme(id) {
    return (dispatch) => {
        fetch(`https://memes-vs-gifs-api.herokuapp.com/memes/${id}`, {
            method: "DELETE"
        })
            .then(res => res.json())
